...
public class LinkedListStepDef {
	...
	public void user_clicks_the_getting_started_button_in_panel(String panelName) {
		...
		// homePage.clickGetStartedButtonOfGivenDsType(panelName);
	...
	public void user_is_on_linked_list_page() {
		...
		String expectedlinklistUrl = "/linked-list/"; // keep this in constant class inside Utils package
		Assert.assertTrue(linkedListPage.getLinkedListPageURL().endsWith(expectedlinklistUrl));
		LoggerFactory.getLogger().info("User is on the Linked List page");
	}

	@Then("User should see {string} header for Linkedlist page")
	public void user_should_see_header_for_linkedlist_page(String expectedHeaderLinkeList) {
		Assert.assertEquals(linkedListPage.getHeaderTitle(), expectedHeaderLinkeList);
		LoggerFactory.getLogger().info("User see the Linked List Header");
	}

	@Then("User should see {string} having link for Linked List page")
	public void user_should_see_having_link_for_linked_list_page(String string) {
	...
	public void user_should_see_button_for_the_respective_linklist_page(String TryHereBtnTxt) {

		Assert.assertEquals(linkedListPage.tryHereBtn(), TryHereBtnTxt);
	...
	public void user_should_be_redirected_to_page_containing(String expectedLink, String partialLinkText) {
		...
		Assert.assertTrue(driver.getCurrentUrl().toLowerCase().contains(partialLinkText.toLowerCase()));
