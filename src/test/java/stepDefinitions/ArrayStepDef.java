...
public class ArrayStepDef {
	...
	public void user_is_on_Array_page(String pageName) {
		...
		case "array":
			arrayPage = new ArrayPage(driver);
			Assert.assertTrue(arrayPage.isOnArrayPage(), "User is NOT on Array page");
			break;

		case "practice questions":
			pqPage = new ArrayPracticePage(driver);
			Assert.assertTrue(pqPage.isOnPracticeQuestionsPage(), "User is NOT on Practice Questions page");
			break;
	...
	public void user_should_see_header_for_array_page(String expectedHeader) {
		Assert.assertTrue(arrayPage.isHeaderVisible(), "Array page header is NOT visible");
		Assert.assertEquals(arrayPage.getHeaderText(), expectedHeader);
	}

	@Then("User should see Topics Covered header")
	public void user_should_see_topics_covered_header(String expectedHeader) {
		Assert.assertTrue(arrayPage.isTopicsCoveredHeaderVisible(), "Topics Covered header is NOT visible");
		Assert.assertEquals(arrayPage.getTopicsCoveredHeaderText(), expectedHeader);
	}
	...
	public void user_should_see_link_of_array_page(String topicName) {
		...
		case "Arrays in Python":
			Assert.assertTrue(arrayPage.isArraysInPythonDisplayed());
			break;
		case "Arrays Using List":
			Assert.assertTrue(arrayPage.isArraysUsingListDisplayed());
			break;
		case "Basic Operations in Lists":
			Assert.assertTrue(arrayPage.isBasicOperationsInListDisplayed());
			break;
		case "Applications of Array":
			Assert.assertTrue(arrayPage.isApplicationsOfArrayDisplayed());
			break;
		default:
	...
	public void user_should_see_button_on_the_array_page(String buttonText) {
		Assert.assertTrue(arrayPage.isTryHereButtonVisible(), "Try Here button is NOT visible");
	}
	...
	public void user_should_see_link_of_practice_question_page(String topicName) {
		...
		case "Search the array":
			Assert.assertTrue(pqPage.isSearchTheArrayDisplayed());
			break;
		case "Max Consecutive Ones":
			Assert.assertTrue(pqPage.isMaxConsecutiveOnesDisplayed());
			break;
		case "Find Numbers with Even Number of Digits":
			Assert.assertTrue(pqPage.isFindNumbersWithEvenNumberOfDigitsDisplayed());
			break;
		case "Squares of a sorted Array":
			Assert.assertTrue(pqPage.isSquaresOfSortedArrayDisplayed());
			break;
		default:
			throw new RuntimeException("Topic not found: " + topicName);
		}
	...
	public void user_should_see_question_run_and_submit_buttons() {
		Assert.assertTrue(pqPage.isQuestionTextVisible());
		Assert.assertTrue(pqPage.isRunButtonVisible());
		Assert.assertTrue(pqPage.isSubmitButtonVisible());
	}
	...
	public void user_is_on_practice_question_editor(String questionName) {
		...
		Assert.assertTrue(pqPage.isEditorVisible(), "Editor not visible for " + questionName);
	}
	...
	public void user_should_able_to_see(String expectedResult) {
		...
		case "output displayed":
			Assert.assertTrue(pqPage.isOutputVisible());
			break;
		case "error message":
			Assert.assertTrue(pqPage.isErrorMessageVisible());
			break;
		case "alert with error message":
			Assert.assertTrue(pqPage.isAlertPresent());
			break;
		default:
